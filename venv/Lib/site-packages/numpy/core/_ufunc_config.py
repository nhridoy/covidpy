...
def setbufsize(size):
    ...
    """
    if size > 10e6:
        raise ValueError(f"Buffer size, {size}, is too big.")
    if size < 5:
        raise ValueError(f"Buffer size, {size}, is too small.")
    if size % 16 != 0:
        raise ValueError(f"Buffer size, {size}, is not a multiple of 16.")
