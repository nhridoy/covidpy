...
_nbo = '<' if sys.byteorder == 'little' else '>'
    ...
    while startindex < len(astr):
        ...
        if startindex < len(astr):
            if space_re.match(astr, pos=startindex):
                startindex = len(astr)
            elif mo := sep_re.match(astr, pos=startindex):
                startindex = mo.end()
        ...
        if order in ('|', '=', _nbo):
            order = ''
        dtype = order + dtype
        newitem = dtype if (repeats == '') else (dtype, ast.literal_eval(repeats))
...
class _ctypes:
    ...
    def shape_as(self, obj):
        ...
        """
        return None if self._zerod else (obj*self._arr.ndim)(*self._arr.shape)
    ...
    def strides_as(self, obj):
        ...
        """
        return None if self._zerod else (obj*self._arr.ndim)(*self._arr.strides)
...
def __dtype_from_pep3118(stream, is_subdtype):
    ...
    # Parse spec
    while stream and not stream.consume('}'):
        # Sub-arrays (1)
        shape = None
        if stream.consume('('):
        ...
        if itemsize_str := stream.consume_until(lambda c: not c.isdigit()):
...
def _ufunc_doc_signature_formatter(ufunc):
    ...
    if ufunc.nout == 0:
        ...
        out_args = '[, {positional}], / [, out={default}]'.format(
            positional=', '.join(f'out{i + 1}' for i in range(ufunc.nout)),
...
def npy_ctypes_check(cls):
    ...
    try:
        ...
        ctype_base = cls.__mro__[-3] if IS_PYPY else cls.__mro__[-2]
