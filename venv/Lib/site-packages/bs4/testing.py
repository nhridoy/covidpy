    ...
    def linkage_validator(self, el, _recursive_call=False):
        ...
        for idx, child in enumerate(el.contents):
            ...
            if idx == 0:
                ...
                ), f"Bad previous_sibling\nNODE: {child}\nPREV {child.previous_sibling}\nEXPECTED {el.contents[idx - 1]}"

                assert (
                    el.contents[idx - 1].next_sibling is child
                ), f"Bad next_sibling\nNODE: {el.contents[idx - 1]}\nNEXT {el.contents[idx - 1].next_sibling}\nEXPECTED {child}"


                if last_child is not None:
                    assert (
                        child.previous_element is last_child
                    ), f"Bad previous_element\nNODE: {child}\nPREV {child.previous_element}\nEXPECTED {last_child}\nCONTENTS {child.parent.contents}"

                    assert (
                        last_child.next_element is child
                    ), f"Bad next_element\nNODE: {last_child}\nNEXT {last_child.next_element}\nEXPECTED {child}"
            ...
            if isinstance(child, Tag) and child.contents:
                ...
                ), f"Bad next_sibling\nNODE: {descendant}\nNEXT {descendant.next_sibling}\nEXPECTED None"


            # Mark last child as either the bubbled up descendant or the current child
            last_child = descendant if descendant is not None else child
        ...
        if _recursive_call or child is None:
        ...
        while True:
            if target is None:
                ...
                ), f"Bad next_element\nNODE: {child}\nNEXT {child.next_element}\nEXPECTED None"

                break
            elif target.next_sibling is not None:
                assert (
                    child.next_element is target.next_sibling
                ), f"Bad next_element\nNODE: {child}\nNEXT {child.next_element}\nEXPECTED {target.next_sibling}"
...
class HTMLTreeBuilderSmokeTest(object):
    ...
    def _document_with_doctype(self, doctype_fragment, doctype_string="DOCTYPE"):
        ...
        doctype = f'<!{doctype_string} {doctype_fragment}>'
        markup = doctype + '\n<p>foo</p>'
